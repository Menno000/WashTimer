...
* Date: 10/6/2015, Code base ported from flashlight 5, created pin mappings. Finished the
* algorithum for the timer system.
*******************************************************************************/
...
char SWITCH_A_FLAG = 0;
char SWITCH_B_FLAG = 0;
char timer_state = 0;
...
__interrupt void Timer_A (void)
{
   ...
   if (last_user_input_ms >= 60000) //One minute since last user input
   {
	   ...
	   if (last_user_input_min == 3) //3 minutes with no user input, set to mode 2 (flashlight off unless button press, to save battery)
	   {
   ...
   if (timers[BUTTON_TIMER].Enabled == 1)
   {
      if (timers[BUTTON_TIMER].NextTime == ms_ticks) //Timer expired
	  {
    	  ...
    	  if ((P2IN & SWITCH_A) == SWITCH_A)
    	  {
    		  SWITCH_A_FLAG = 1;
    	  }
    	  if ((P2IN & SWITCH_B) == SWITCH_B)
    	  {
    		  SWITCH_B_FLAG = 1;
    	  }
	  }
      ...
      if (timers[STROBE_TIMER].NextTime == ms_ticks)
      {
		 ...
		 else if (timer_state == STATE_SLEEP_MODE)
		 {
             if (leds_on == 0)
             {
            	 P2OUT |= RED_LED;
            	 leds_on = 1;
            	 StartTimer(STROBE_TIMER, 100);
             }
             else
             {
            	 P2OUT &= ~RED_LED;
            	 leds_on = 0;
            	 StartTimer(STROBE_TIMER, 2000);
             }
   ...
   if (timers[UI_TIMER].Flag) //Time to check for user input
   {
	   ...
	   if (BUTTON_LEFTFLAG) //Starts the timer
	   {
		   ...
		   if (timer_state == STATE_STANDBY)
		   {
			   ...
			   if (SWITCH_A_FLAG)
			   {
			      timer_run_iterations = 300; //For a 30 second timer
			      strobe_ms = 100;
			   }
			   else if (SWITCH_B_FLAG)
			   {
				  timer_run_iterations = 300; //For a 1 minute timer
				  strobe_ms = 300;
			   }
